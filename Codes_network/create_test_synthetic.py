...
if case ==1:
    ...
    for index_image in [0,1,2,3,4,5]:
        ### --- Import data 
        Dir = os.path.join(Init_Dir, 'Simulate_data')
        print('Import data ...')
        depth = sio.loadmat(os.path.join(Dir,'RAW', 'depth_data.mat'))['depth_data_2005']
        intensity = sio.loadmat(os.path.join(Dir,'RAW', 'intensity_data.mat'))['intensity_data_2005']
        I_up = np.squeeze(depth[0,index_image])
        intensity_image = np.squeeze(intensity[0,index_image])
        ...
        histogram_step_1, histogram_step_2  = second_peak_histogram(histogram,level)
        ...
        save_path = os.path.join(Dir,'Middlebury_'+str(index_image)+'_ppp'+str(ppp)+'_SBR'+str(SBR)+'_multiple_level'+str(level))
