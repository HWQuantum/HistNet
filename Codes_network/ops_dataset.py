...
def center_of_mass(Histogram_training_depth_LR, type):
    ...
    for index in range(nb_images):
        ...
        #print('Attention change from 15 to 16')
...
def second_peak_histogram(histogram,level):

    Nx_LR = int(histogram.shape[0])
    Ny_LR = int(histogram.shape[1])
    Nbins = histogram.shape[2]
    
    close_histogram = np.zeros((Nx_LR,Ny_LR,Nbins))
    far_histogram = np.zeros((Nx_LR,Ny_LR,Nbins))
    ...
    for i in range(Nx_LR):
            for j in range(Ny_LR):
                ...
                if max(new_histogram[i,j,:]) < b + level * np.sqrt(b):
                    close_histogram[i,j,:] = histogram[i,j,:]
                    far_histogram[i,j,:] = np.zeros(Nbins)
                else :
                    close_histogram[i,j,:] = histogram[i,j,:]
                    far_histogram[i,j,:] = new_histogram[i,j,:]
        
    return close_histogram, far_histogram

def second_peak_histogram_real(histogram_initial, type, level):
